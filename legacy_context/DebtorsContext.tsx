...
type DebtorsContextType = {
  ...
  addDebt: (id: string, amount: number, dueDate?: number) => void;
};
...
export function DebtorsProvider({ children }: { children: ReactNode }) {
  ...
  const addDebt = (id: string, amount: number, dueDate?: number) => {
    setDebtors((prev) =>
      prev.map((d) =>
        d.id === id
          ? {
              ...
              dueDate: dueDate
                ...
                : d.dueDate,
              status: d.status === "Settled" ? "Unpaid" : d.status,
            }
          : d,
      ),
    );
  ...
  return (
    <DebtorsContext.Provider value={{ debtors, addDebtor, addDebt }}>
      {children}
